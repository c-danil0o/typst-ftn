...
Десктоп апликација представља централну компоненту система GymMan, осмишљену као главни алат за администраторе и раднике у фитнес центру. За њену израду изабран је радни оквир
Tauri#footnote[https://v2.tauri.app/], који омогућава развој вишеплатформских (енгл. _cross-platform_) апликација комбиновањем перформанси и сигурности програмског језика Rust са флексибилношћу модерних веб технологија за
израду корисничког интерфејса. У поређењу са алтернативама као што је Electron#footnote[https://www.electronjs.org/], Tauri нуди значајне предности у виду мање потрошње меморије, мање величине финалне апликације и директнијег приступа
системским ресурсима, што га је учинило идеалним избором за овај пројекат @tauri@electron-tauri.

Архитектура десктоп апликације подељена је на две основне целине: позадински део (енгл. _backend_) реализован у Rust-у и кориснички интерфејс (енгл. _frontend_) реализован помоћу Svelte#footnote[https://svelte.dev/] оквира.
...
За интеракцију са SQLite @sqlite базом података користи се библиотека SQLx#footnote[https://github.com/launchbadge/sqlx], која пружа типски безбедне (енгл. _type-safe_) SQL упите верификоване током компајлирања.
...
Систем користи асинхрони Tokio#footnote[https://tokio.rs/] _runtime_ за извршавање дуготрајних задатака у позадини, независно од главног UI тока. Кључни позадински процеси су:
...
3.  Генерише `latest.json` манифест: Акција `tauri-action` унутар радног тока (енгл. _workflow_) прикупља информације о новој верзији,
...
```
Овај механизам омогућава апликацији да провери `latest.json` фајл на GitHub-у и утврди да ли постоје нове верзије. Када се открије нова верзија, апликација преузима одговарајући инсталациони пакет и његов дигитални потпис.
...
Кориснички интерфејс (фронтенд) десктоп апликације представља компоненту са којом корисници (администратори и радници) директно интерагују. За његову реализацију изабран је SvelteKit#footnote[https://kit.svelte.dev/],
...
- `src/lib/`: Централни директоријум за сав дељени код.
  ...
  - `src/lib/models/`: Садржи спецификацију TypeScript објеката који одговарају типовима за пренос података (DTO, енгл. Data-Transfer-Object) дефинисаним у Rust позадинском делу. Ово обезбеђује типску безбедност приликом комуникације између фронтенда и бекенда.
...
Стилизовање апликације урађено је помоћу Tailwind CSS#footnote[https://tailwindcss.com/]. За сложеније UI елементе као што су модални дијалози, табеле са пагинацијом и сортирањем, и падајуће листе, коришћена је библиотека shadcn-svelte#footnote[https://www.shadcn-svelte.com/].
Ова библиотека представља регистар прилагодљивих и приступачних (енгл. accessible) компоненти које се лако интегришу и модификују. Компоненте се инсталирају појединачно, што омогућава минималну величину пакета и боље перформансе.
...
За валидацију корисничког уноса у формама користи се библиотека Zod#footnote[https://zod.dev/] за дефинисање шема података и правила валидације, у комбинацији са Superforms#footnote[https://superforms.rocks/] библиотеком
...
Подршка за више језика (српски, енглески) реализована је помоћу ParaglideJS#footnote[https://inlang.com/m/gerre34r/library-paraglide-js]. Овај алат компајлира преводе из JSON фајлова у типски сигурне
...
За реализацију ове две различите потребе, изабране су две одвојене _cloud_ платформе, свака оптимизована за свој задатак. За систем резервних копија коришћени су AWS#footnote[https://aws.amazon.com] сервиси,
док је за синхронизацију података и подршку за мобилну апликацију изабрана Supabase#footnote[https://supabase.com/] платформа. У наставку ће бити детаљно описана архитектура и имплементација оба система.
...
2.  *AWS Lambda*: Представља _serverless_ рачунарску платформу. Имплементиране су три одвојене Lambda функције написане у програмском језику Python, свака са специфичном наменом:
    - `upload-backup`: Прихвата `POST` захтев са компримованим фајлом, верификује аутентификациони токен и смешта фајл у одговарајући S3 префикс.
...
За развој је изабран Flutter#footnote[https://flutter.dev/] радни оквир компаније Google, који омогућава креирање нативних апликација за Android и iOS платформе у програмском језику Dart.
...
- Позивање RPC функције: Уместо да апликација прави сложене упите, она позива једну, претходно дефинисану функцију базе података: `get_my_relevant_membership`. Ова функција на серверу извршава сву
...
Веб апликација за управљање лозинком представља још једну једноставну компоненту GymMan система. Њена основна улога је да послужи као сигуран повезник између позива који члан добије путем е-поште и његовог успешног активирања налога за мобилну апликацију. Ова компонента решава два кључна сценарија:
...
За развој веб апликације изабран је SvelteKit, исти радни оквир који се користи и за фронтенд десктоп апликације. За хостинг је изабрана Netlify#footnote[https://www.netlify.com/] платформа.
...
6.  Завршетак процеса: Након успешног ажурирања, сесија се уништава, а кориснику се приказује порука која га упућује да преузме мобилну апликацију и да се пријави користећи своју е-пошту и нову лозинку.
...
(способност да мејлови не заврше у _spam_ фолдеру), систем је конфигурисан да користи спољни SMTP сервис. За ову сврху изабран је Resend#footnote[https://resend.com/] сервис за е-поруке.
