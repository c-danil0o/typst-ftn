...
системским ресурсима, што га је учинило идеалним избором за овај пројекат @tauri.

Архитектура десктоп апликације подељена је на две основне целине: позадински део (енгл. _backend_) реализован у Rust-у и кориснички интерфејс (енгл. _frontend_) реализован помоћу #link("https://kit.svelte.dev/")[SvelteKit] оквира.
...
Кориснички интерфејс (фронтенд) десктоп апликације представља компоненту са којом корисници (администратори и запослени) директно интерагују. За његову реализацију изабран је ,
савремени радни оквир за израду веб апликација. Кључна предност Svelte-а лежи у његовом компајлерском приступу: уместо да се ослања на виртуелни објектни модел документа (DOM, енгл. _Document Object Model_), Svelte компајлира компоненте у оптималан,
...
праве са Tauri радним оквиром. Интерфејс је потпуно прилагодљив величини прозора, омогућавајући рад на различитим уређајима и резолуцијама.
...
- `src/lib/`: Централни директоријум за сав дељени код.
  ...
  - `src/lib/models/`: Садржи TypeScript типове који су аутоматски који одговарају DTO (енгл. Data-Transfer-Object) типовима дефинисаним у Rust позадинском делу. Ово обезбеђује типску безбедност приликом комуникације између фронтенда и бекенда.
