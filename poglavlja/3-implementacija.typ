...
кључне механизме система.

== Имплементација десктоп апликације

Десктоп апликација представља централну компоненту система _GymMan_, осмишљену као главни алат за администраторе и запослене у фитнес центру. За њену израду изабран је радни оквир
#link("https://tauri.app/")[Tauri], који омогућава развој "cross-platform" апликација комбиновањем перформанси и сигурности програмског језика Rust са флексибилношћу модерних веб технологија за
израду корисничког интерфејса. У поређењу са алтернативама као што је _Electron_, Tauri нуди значајне предности у виду мање потрошње меморије, мање величине финалне апликације и директнијег приступа
системским ресурсима, што га је учинило идеалним избором за овај пројекат.

Архитектура десктоп апликације подељена је на две основне целине: позадински део (енгл. _backend_) реализован у Rust-у и кориснички интерфејс (енгл. _frontend_) реализован помоћу SvelteKit оквира.
...
је Rust због својих гаранција сигурности меморије, високих перформанси и одличног екосистема за конкурентно програмирање, што је од кључног значаја за асинхроне задатке попут синхронизације и прављења резервних копија.
...
Централно стање апликације (енгл. _state_) дефинисано је у структури `AppState`, која се иницијализује приликом покретања и дели између свих компоненти путем Tauri механизма за управљање стањем. Ова структура садржи конекционе пулове за локалну (и опционо, удаљену) базу података, као и конфигурациона подешавања апликације.
...
```
Овај приступ елиминише потребу за интерним HTTP сервером и омогућава брзу и сигурну комуникацију између два дела апликације.

=== Управљање базом података и миграције

За интеракцију са SQLite базом података користи се библиотека #link("https://github.com/launchbadge/sqlx")[SQLx], која пружа "type-safe" SQL упите верификоване током компајлирања.
...
Ови задаци се покрећу приликом стартовања апликације као засебни асинхрони задаци, користећи `tokio::spawn`.
Сви процеси се логују у структурисаном формату користећи библиотеку `tracing`.

=== Конфигурација и чување подешавања

Подешавања апликације као што су _Supabase_ API кључ, сервер резервних копија, интервал синхронизације и избор језика поставља админстратор и она се чувају се у посебном JSON фајлу (`app_settings.json`) у апликационом директоријуму добијеном преко `tauri::api::path::app_data_dir()`.
...
Како би се корисницима омогућило једноставно преузимање и редовно ажурирање апликације, систем _GymMan_ користи аутоматизоване процесе за израду, објављивање и дистрибуцију нових верзија. Овај приступ осигурава да сваки корисник увек има приступ најновијим функционалностима и сигурносним исправкама уз минималан напор.

==== Аутоматизована израда на GitHub платформи

Процес објављивања нових верзија је у потпуности аутоматизован помоћу _GitHub Actions_. Сваки пут када се на репозиторијум постави нови "tag" (нпр. `v1.0.1`), аутоматски се покреће процес који:
...
2.  *Дигитално потписује пакет*: Користећи сигурносни кључ сачуван у _GitHub Secrets_, сваки инсталациони пакет добија дигитални потпис (`.sig` фајл). Овај потпис гарантује да ажурирање долази од аутентичног извора и да није мењано.
3.  *Генерише `latest.json` манифест*: Кључни корак у овом процесу је аутоматско генерисање `latest.json` датотеке. `tauri-action` акција унутар _workflow_-а прикупља информације о новој верзији,
...
4.  *Објављује на GitHub Releases*: На крају, сви пакети, њихови потписи, и генерисани `latest.json` фајл се аутоматски објављују као артефакти у оквиру новог _release_-а на GitHub-у.

Овај CI/CD (континуирана интеграција и испорука) процес елиминише ручни рад, смањује могућност грешке и убрзава испоруку нових верзија.

==== Механизам за аутоматско ажурирање

Десктоп апликација има уграђен механизам за аутоматско ажурирање, реализован помоћу `tauri-plugin-updater` плагина. Конфигурација овог плагина у `tauri.conf.json` фајлу је подешена да директно комуницира са GitHub _release_-ом.
```json
"plugins": {
  "updater": {
    "active": true,
...
```
Овај механизам омогућава апликацији да провери `latest.json` фајл на GitHub-у за нове верзије. Када се открије нова верзија, апликација преузима одговарајући инсталациони пакет и његов дигитални потпис.
...
Кориснички интерфејс (фронтенд) десктоп апликације представља компоненту са којом корисници (администратори и запослени) директно интерагују. За његову реализацију изабран је #link("https://kit.svelte.dev/")[SvelteKit],
савремени радни оквир за израду веб апликација. Кључна предност Svelte-а лежи у његовом компајлерском приступу: уместо да се ослања на виртуелни DOM, Svelte компајлира компоненте у оптималан,
императивни JavaScript код који директно манипулише DOM-ом. Ово резултира изузетно брзим апликацијама са минималним "overhead"-ом, што га чини идеалним за ресурсно ефикасне десктоп апликације попут оних које се
праве са Tauri-јем. Интерфејс је потпуно прилагодљив величини прозора, омогућавајући рад на различитим уређајима и резолуцијама.
...
- `src/routes/`: Овај директоријум садржи структуру свих страница и рута у апликацији. SvelteKit користи "file-system based routing", где структура фајлова директно одговара URL путањама. На пример, фајл `src/routes/members/[id]/+page.svelte` дефинише страницу за приказ детаља појединачног члана.
- `src/lib/`: Централни директоријум за сав дељени код.
  - `src/lib/components/`: Садржи реискористиве Svelte компоненте као што су форме, табеле, дијалози, графикони и дугмад.
  - `src/lib/stores/`: Дефинише глобално стање апликације коришћењем Svelte "stores". Овде се чувају подаци о улогованом кориснику, статусу синхронизације, изабраној теми (светла/тамна) и језику.
  - `src/lib/models/`: Садржи TypeScript типове који су аутоматски који одговарају DTO типовима дефинисаним у Rust позадинском делу. Ово обезбеђује типску безбедност приликом комуникације између фронтенда и бекенда.
...
Пример једноставног store-а за тренутног корисника:
```ts
...
Свака промена у store-у аутоматски се рефлектује у компонентама које га користе, што омогућава природну реактивност интерфејса без додатних логичких слојева.
...
- Валидацију у реалном времену, док корисник куца.
...
Иако је _GymMan_ систем дизајниран са "offline-first" архитектуром као основном премисом, инфраструктура у облаку (енгл. _cloud_) игра важну, двоструку улогу у његовом функционисању. Прва улога је обезбеђивање сигурног и
поузданог механизма за прављење резервних копија (енгл. _backup_) комплетне локалне базе података, што је од великог значаја за очување података у случају хардверског квара или других непредвиђених догађаја.
Друга улога је подршка за опциони "online" режим рада, где се подаци асинхроно синхронизују са централном базом како би се омогућила функционалност мобилне апликације.

За реализацију ове две различите потребе, изабране су две одвојене "cloud" платформе, свака оптимизована за свој задатак. За систем резервних копија коришћени су сервиси #link("https://aws.amazon.com/")[Amazon Web Services (AWS)
...
Овај процес мора бити безбедан, ефикасан и једноставан за конфигурацију са корисничке стране. Изабрана је "serverless" архитектура на AWS-у, која елиминише потребу за одржавањем сервера и нуди
...
4.  *AWS IAM (Identity and Access Management)*: Користи се за дефинисање прецизних дозвола. Креирана је посебна IAM улога за Lambda функције која им даје минималне потребне привилегије: дозволу за упис и читање објеката унутар S3 "bucket"-а и дозволу за упис логова у Amazon CloudWatch.

==== Процес слања и враћања резервне копије

Процес са стране десктоп апликације (Rust бекенд) је следећи:

1.  *Креирање резервне копије*: На захтев корисника, апликација користи `VACUUM INTO` SQL команду да креира конзистентан "snapshot" локалне SQLite базе у привремени фајл. Овај приступ је сигуран за коришћење док апликација ради, јер исправно рукује WAL (_Write-Ahead Logging_) режимом.

2.  *Слање на API*: Привремени фајл се чита у меморију, а затим се шаље као бинарни податак путем `POST` HTTP захтева на API Gateway "endpoint". Захтев у заглављу (енгл. _header_) садржи и сигурносни токен који аутентификује клијента.

3.  *Враћање података (Restore)*: Процес враћања је вишефазни како би се осигурао интегритет података. Прво се преузима листа доступних верзија. Након што корисник одабере верзију, апликација преузима компримовани фајл, затвара конекцију ка локалној бази, брише старе `.sqlite-wal` и `.sqlite-shm` фајлове, и тек онда замењује главни `.sqlite` фајл са преузетим.

==== Конфигурација са корисничке стране

Да би систем био флексибилан, корисник (власник теретане) у подешавањима апликације уноси јединствени URL за бекап. Овај URL садржи _endpoint_ и сигурносни токен, форматиран на следећи начин:
```url
https://<api_gateway_id>.execute-api.<region>.amazonaws.com/prod#<backup_token>
```
Апликација парсира овај URL како би добила базни URL за API позиве и токен за аутентификацију. Токен се генерише приликом првог покретања апликације као јединствени UUID и приказује се кориснику. Уколико корисник жели да користи исти бекап систем на другом рачунару (пренесе систем на други рачунар), потребно је само да унесе исти URL.

=== _Online_ режим и синхронизација са Supabase платформом

_Оnline_ режим представља сложену архитектуру која омогућава континуирану синхронизацију података и функционалност мобилне апликације. За ову сврху, изабрана је Supabase платформа због своје интегрисане понуде која укључује PostgreSQL базу, систем за аутентификацију и аутоматски генерисан PostgREST API.
...
1.  *PostgreSQL база података*: Централна база података чија шема одражава поједностављену структуру локалне SQLite базе. Иако је могуће, формалне _foreign key_ релације нису дефинисане како би се поједноставила
    логика _upsert_ (комбинација _update_ и _insert_) операција из _Sync Worker_-а.
...
1.  *Хватање промена*: Свака `INSERT`, `UPDATE` или `DELETE` операција у локалној SQLite бази, путем SQL тригера, аутоматски креира запис у `pending_changes` табели.
...
3.  *"Claim-then-process" механизам*: Да би се спречило да више _worker_-а (у теорији) обрађује исти запис, _worker_ прво атомски ажурира статус записа у _processing_. Тек након тога шаље податке на Supabase PostgREST API.
...
Овај избор доноси значајне предности у виду бржег развоја, конзистентног корисничког искуства на обе платформе и приступа богатом екосистему готових компоненти.
...
│ └── widgets/ # Реискористиве UI компоненте
├── home/
...
Асинхроно учитавање података: Оба таба користе `FutureBuilder` виџет. Ово омогућава да се приликом учитавања података са мреже прикаже индикатор учитавања (енгл. `loading spinner`), а у случају грешке, јасна порука кориснику. Ово спречава "замрзавање" интерфејса и побољшава корисничко искуство.
...
Веб апликација за управљање лозинком још једну једноставну компоненту _GymMan_ система. Њена основна улога је да послужи као сигуран повезник између позива који члан добије путем е-поште и његовог успешног активирања налога за мобилну апликацију. Ова компонента решава два кључна сценарија:
...
Netlify је изабран због своје једноставности, бесплатног _tier_-а за мање пројекте и одличне интеграције са Git-ом, што омогућава континуирану испоруку (енгл. _Continuous Deployment_).
...
6.  *Завршетак процеса*: Након успешног ажурирања, кориснику се приказује порука која га упућује да преузме моблину апликацију и да се пријави користећи своју е-пошту и нову лозинку.
