...
Кроз овај рад представљен је процес пројектовања, развоја и имплементације техничког решења GymMan, намењеног управљању фитнес центрима. Рад је започет анализом изазова са којима се сусрећу мањи и
...
Као одговор на ове проблеме, пројектована је и реализована хибридна архитектура која комбинује најбоље од оба света. Срж система чини _cross-platform_ десктоп апликација са _offline-first_ приступом,
...
2.  *Имплементирана је "offline-first" архитектура* као темељ система. Коришћењем локалне SQLite базе података као примарног извора података, постигнути су велика поузданост и перформансе.
...
3.  *Дизајниран је и имплементиран робустан механизам за асинхрону синхронизацију*. Коришћењем SQL тригера и _outbox_ обрасца, свака промена у локалној бази се аутоматски бележи.
    Позадински процес у Rust-у, користећи "claim-then-process" логику, поуздано шаље ове промене на централну PostgreSQL базу на Supabase платформи, уз механизме за поновне покушаје и руковање грешкама.
    Овим је имплементиран образац евентуалне конзистентности, омогућавајући поуздану репликацију података уз минималан утицај на корисничко искуство.
...
- Флексибилност: Систем може да функционише у три различита режима: потпуно офлајн, офлајн са резервном копијом у облаку, или потпуно онлајн са синхронизацијом. Ово омогућава да се прилагоди потребама и техничким могућностима различитих
   фитнес центара.
...
- Ниски трошкови одржавања: Ослањање на _serverless_ архитектуру (AWS Lambda, Supabase) и _open-source_ технологије значајно смањује оперативне трошкове у поређењу са традиционалним серверским решењима.
- Модерно корисничко искуство: Комбинација интуитивне десктоп апликације и пратеће мобилне апликације подиже квалитет услуге фитнес центра.

Мане и ограничења:

- Сложеност синхронизације: Иако је имплементирани _outbox_ образац робустан, он подразумева једносмерну синхронизацију (са локалне на удаљену базу).
...
У закључку, овај рад је успешно демонстрирао како се комбинацијом модерних _cross-platform_ технологија и промишљеном архитектуром може креирати свеобухватно, поуздано и скалабилно софтверско решење.
